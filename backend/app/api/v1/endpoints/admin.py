...
async def get_dashboard_stats(
    ...
    active_users_24h = await db.scalar(
        ...
        .where(Conversation.last_message_at >= last_24h)
    )
    active_users_7d = await db.scalar(
        select(func.count(distinct(Conversation.user_id)))
        .where(Conversation.last_message_at >= last_7d)
    )
    active_users_30d = await db.scalar(
        select(func.count(distinct(Conversation.user_id)))
        .where(Conversation.last_message_at >= last_30d)
    )
    ...
    doc_status = {
        status: count for status, count in doc_status_result.all()
    }
    ...
    return {
        ...
        "query_metrics": {
            ...
            "avg_feedback_rating": float(query_metrics.avg_feedback_rating or 0) if query_metrics.avg_feedback_rating else None
        },
...
async def get_query_analytics(
    ...
    feedback_distribution = {
        int(row.feedback_rating): row.count
        for row in feedback.all()
    }
